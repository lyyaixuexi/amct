...
演示使用AscendCL执行模型推理一般步骤的基础示例应用，以ResNet50网络模型为例进行模型推理，实现对物体进行分类，并给出TOP5类别置信度和相应类别信息
...
## 样例运行
  ...
  - 数据准备
    ...
    ```    
    cd $HOME/samples/inference/modelInference/sampleResnetQuickStart/data
  ...
  - ATC模型转换
    ...
    cd $HOME/samples/inference/modelInference/sampleResnetQuickStart/model
  ...
  - 样例编译
    ...
    ```
    cd $HOME/samples/inference/modelInference/sampleResnetQuickStart/scripts
  ...
  - 样例结果展示
    ...
    执行成功后，在屏幕上的关键提示信息示例如下，提示信息中的top1-5表示图片置信度的前5种类别、index表示类别标识、value表示该分类的最大置信度，class表示所属类别。这些    值可能会根据版本、环境有所不同，请以实际情况为准：


    ```
    [INFO] The sample starts to run
    [INFO]  top 1: index[162] value[0.902209] class[beagle]
    [INFO]  top 2: index[161] value[0.096590] class[bassetbasset hound]
...
## 更新说明
  | 时间 | 更新事项 |
|----|------|
| 2023/02/17 | 新增sampleResnetQuickStart/README.md |
